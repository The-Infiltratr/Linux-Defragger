...
from .base import BackendError, BackendInfo, CAP_ANALYSE, CAP_MAP, Reader, aggregate_ranges, u32le, u64le

INFO = BackendInfo("apfs", "Apple APFS", ("apfs",), CAP_ANALYSE | CAP_MAP, "summary")
...
class APFSBackend:
    ...
    def map(self, path: str, cells: int) -> dict:
        with Reader(path) as reader:
            sb = reader.read(0, 4096)
            if sb[32:36] != b"NXSB":
                raise BackendError("not an APFS container")
            block_size = u32le(sb, 36)
            ...
            return aggregate_ranges(
                block_count, cells, block_size, "apfs",
                [(0, 1, 1), (1, block_count, 2)], "summary",
                details={"container_uuid": sb[72:88].hex(), "note": "APFS spaceman allocation map not yet decoded"},
            )
