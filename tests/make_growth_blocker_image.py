...
# Purpose: Create a compact FAT32 layout whose fragmented chains block Growth Defrag targets.

import struct
import sys
from pathlib import Path

out = Path(sys.argv[1] if len(sys.argv) > 1 else "growth-blocker-fat32.img")
bps = 512
spc = 1
reserved = 32
fats = 2
...
for index, (name, chain) in enumerate(chains.items()):
    dirent(index * 32, name, chain[0], len(chain) * bps)
root[len(chains) * 32] = 0
...
with out.open("r+b") as f:
    ...
    for file_index, chain in enumerate(chains.values()):
        for cluster_index, cluster in enumerate(chain):
            marker = 0x41 + file_index * 4 + cluster_index
            f.seek(coff(cluster)); f.write(bytes([marker]) * bps)
